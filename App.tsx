...
const App: React.FC = () => {
  ...
  const syncLocationToServer = useCallback((latitude: number, longitude: number) => {
    ...
    // Chỉ log để debug, việc mở khóa UI không phụ thuộc vào kết quả API này
    ApiService.saveLocation(payload).then(res => {
      if (res.success) console.debug("Sync vị trí thành công lên Server");
    });
  }, []);
  ...
  useEffect(() => {
    ...
    const startTracking = () => {
      if ("geolocation" in navigator) {
        // Lấy vị trí hiện tại ngay lập tức lần đầu
        navigator.geolocation.getCurrentPosition(
          ...
          (error) => {
            ...
            // Nếu lỗi lần đầu, vẫn để watchPosition tiếp tục cố gắng
          },
        ...
        watchId = navigator.geolocation.watchPosition(
          (position) => {
            // CRITICAL: Cập nhật state local ngay khi có tọa độ để mở Guard
            setIsLocationAcquired(true);
            setLocationError(null);
          ...
          (error) => {
            ...
            if (error.code === error.PERMISSION_DENIED) {
              ...
              msg = "Không thể xác định vị trí của bạn. Vui lòng kiểm tra cài đặt GPS trên thiết bị.";
              // Không set false ngay vì có thể chỉ là mất sóng GPS tạm thời
            } else if (error.code === error.TIMEOUT) {
              // Timeout thì vẫn giữ trạng thái cũ, không block user ngay lập tức nếu trước đó đã có vị trí
            }
          ...
          },
          {
            ...
            timeout: 15000,
            maximumAge: 0 // Bắt buộc lấy dữ liệu mới nhất, không dùng cache
          }
        );
      } else {
  ...
  }, [isLoggedIn, syncLocationToServer]);
  ...
  // Mandatory Location Guard UI component
  const LocationGuard = () => (
      ...
      <div className="relative max-w-lg w-full space-y-8 animate-in zoom-in-95 duration-500">
        ...
        <div className="space-y-4">
          ...
          <p className="text-slate-400 font-medium leading-relaxed">
            Hệ thống <span className="text-indigo-400 font-black">BDSDaily</span> yêu cầu bạn phải chia sẻ vị trí thực tế để truy cập dữ liệu nội bộ.
          </p>
          {locationError && (
        ...
        <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 space-y-4 text-left">
          <div className="flex gap-4 items-start">
            <div className="w-8 h-8 bg-indigo-500/20 rounded-lg flex items-center justify-center text-indigo-400 flex-shrink-0 mt-0.5">
              <Settings className="w-4 h-4" />
            </div>
            <p className="text-xs text-slate-300 font-medium leading-relaxed">
              Vào <span className="text-white font-bold">Cài đặt trình duyệt</span> → <span className="text-white font-bold">Quyền riêng tư</span> → <span className="text-white font-bold">Vị trí</span> và chọn <span className="text-indigo-400">Cho phép</span>.
            </p>
          </div>
          <div className="flex gap-4 items-start">
            <div className="w-8 h-8 bg-amber-500/20 rounded-lg flex items-center justify-center text-amber-400 flex-shrink-0 mt-0.5">
              <AlertTriangle className="w-4 h-4" />
            </div>
            <p className="text-xs text-slate-300 font-medium leading-relaxed">
              Nếu đang dùng điện thoại, hãy đảm bảo nút <span className="text-white font-bold">Vị trí/GPS</span> trong bảng điều khiển nhanh đã được bật.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-black py-5 rounded-2xl shadow-xl shadow-indigo-900/20 transition-all active:scale-95 text-sm tracking-widest uppercase flex items-center justify-center gap-3"
          >
            <RefreshCcw className="w-5 h-5" /> KIỂM TRA LẠI
          </button>
          <button
            onClick={handleLogout}
            className="w-full bg-transparent hover:bg-white/5 text-slate-500 font-black py-4 rounded-2xl transition-all text-[10px] tracking-widest uppercase"
          >
            ĐĂNG XUẤT
          </button>
        </div>
  ...
  // Nếu đã đăng nhập mà chưa lấy được vị trí hoặc đang có lỗi định vị nghiêm trọng (từ chối quyền)
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="mx-auto p-4 md:p-8 lg:p-12 max-w-[1600px]">
          {currentView === 'dashboard' && (
            <Dashboard
              users={users}
              onAddUser={() => { setEditingUser(null); setUserFormData(initialFormState); setIsModalOpen(true); }}
              onEditUser={(user) => { setEditingUser(user); setUserFormData({ ...user, password: '' }); setIsModalOpen(true); }}
          ...
          <div className="relative bg-white rounded-[3rem] shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-white">
            ...
            <form onSubmit={handleSaveUser} className="p-8 space-y-6 max-h-[80vh] overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                    <Fingerprint className="w-3 h-3" /> Mã Nhân Viên
                  </label>
                  <input
                    type="text"
                    placeholder="admin"
                    ...
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 text-sm font-bold"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    type="email"
                    placeholder="email@bdsdaily.com"
                    required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                    <Briefcase className="w-3 h-3" /> Chức vụ
                  </label>
                  <select
                    value={userFormData.role}
                    onChange={e => setUserFormData({ ...userFormData, role: e.target.value })}
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-indigo-500/10 text-sm font-bold appearance-none"
                  >
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button type="submit" className="flex-[2] bg-slate-900 hover:bg-indigo-600 text-white font-black py-4 rounded-2xl shadow-xl transition-all text-xs uppercase tracking-widest active:scale-95">
